...
TEST(DynamicIdxListTest, Count)
{
    ...
    ASSERT_EQ(1, stList.Count(dynamic_idx_list_helper_count_func));

    ASSERT_EQ(2, stList.Count(dynamic_idx_list_helper_class_func_obj()));
}
...
TEST(DynamicIdxListTest, EdgeCondition)
{
    ...
    ASSERT_EQ(1, stEle.begin().index());

    idx1 = stEle.Create(9);
    ASSERT_EQ(0, idx1);
    ASSERT_EQ(2, stEle.Count());

    stEle.Remove(1);
    ASSERT_EQ(1, stEle.Count());
    ASSERT_EQ(0, stEle.begin().index());
