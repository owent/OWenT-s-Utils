...
namespace util
{
    ...
    namespace types
    {
        ...
        {
            strTar += util::EncodeUriComponent(key.c_str(), key.size()) + "=" + util::EncodeUriComponent(value.c_str(), value.size()) + "&";
        }
        ...
        bool ItemArray::Encode(std::string& strOutput, const char* strPrefix) const
        {
            ...
            else // 带下标编码
            {
                for(uIndex = 0; uIndex < m_stData.size(); ++ uIndex)
                {
                    strNewPrefix = strPrePrefix + '[' + util::AnyToQueryString(uIndex) + ']';
                    ret = m_stData[uIndex]->Encode(strOutput, strNewPrefix.c_str()) && ret;
                }
            }
    ...
    bool TQueryString::decodeRecord(const char* strContent, std::size_t uSize)
    {
        ...
        if (uValStart != strOrigin.npos)
        {
            ...
            strValue = util::DecodeUriComponent(strValue.data(), strValue.size());
            strOrigin = strOrigin.substr(0, uValStart);
        }

        strOrigin = util::DecodeUriComponent(strOrigin.data(), strOrigin.size());
